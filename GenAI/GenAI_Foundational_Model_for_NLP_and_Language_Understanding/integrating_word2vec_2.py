...
# This script defines a utility function, "find_similar_words", to identify words most similar to a given target
# word using word embeddings. It computes cosine similarity between word vectors to find the closest matches.
# This helps in NLP tasks like synonym discovery, word clustering, and semantic analysis.
# %%writefile utils.py
...
# After installation, restart the kernel and run all cells from "Importing Required Libraries."
...
# The matrix is then factorized to obtain the word embeddings. For example, if "Man" and "King" co-occure
# many times, their vectors will be simialr.

# Load Stanford GloVe model
# can specify the model name and embedding dimension: GloVe(name='GloVe_model_name', dim=300)
...
# look into the embedding vectors of this large pretrained model for the words in the corpus:
# create an array that returns the index of each word in the GloVe model's vocabulary:
word_to_index = glove_vectors_6B.stoi  # Vocabulary index mapping
word_to_index['team']
# get the embedded vector for a word:
embeddings_Glove6B.weight[word_to_index['team']]
#see how successful the Glove model is in capturing the similarities between words:
...
# loaded the pretrained embeddings for the sample words, let's check if the model can capture the similarity of
# words by finding the distance between words:
...
#a simple hands-on exercise to train a word2vec model using gensim library. In this example,
# you have a small corpus consisting of four sentences.
#Prepare your corpus:
sentences = [["I", "like", "to", "eat", "pizza"],
             ["Pizza", "is", "my", "favorite", "food"],
             ["I", "enjoy", "eating", "pasta"]]
sentences = [[word.lower() for word in sentence] for sentence in sentences]
#The size parameter specifies the dimensionality of the word embeddings (in this case, 100).
# The window parameter determines the size of the context window. The min_count parameter sets the minimum
# frequency of a word to be included in the training process. Finally, the workers parameter controls
...
# The word embeddings obtained from the model would be more meaningful and informative with larger and more diverse
# training data.
# Use the trained model to create a PyTorch embedding layer (just like what you did with the pretrained GloVe model)
# and use it in any task as an embedding layer.
...
#use the embeddings in a task, then. Let's use the pretrained embeddings to classify text data into topics:
#First, build vocab from the pretrained GloVe:
from torchtext.vocab import GloVe,vocab
...
#need to tokenize text. For this you can use pretrained tokenizers from torch:
# Define tokenizer
...
#plot the loss and accuracy for the trained model:
import matplotlib.pyplot as plt
def plot(COST,ACC):
