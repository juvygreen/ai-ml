...
# FNNs, or Multi-Layer Perceptrons, serve as the foundational components for comprehending neural networks in
# natural language processing (NLP). In NLP tasks, FNNs process textual data by transforming it into numerical
# vectors known as embeddings. Subsequently, these embeddings are input to the network to predict language facets,
# such as the upcoming word in a sentence or the sentiment of a text.
...
# Indexing
# TorchText provides tools to tokenize text into individual words (tokens) and build a vocabulary, which maps
# tokens to unique integer indexes. This is a crucial step in preparing text data for machine learning models
...
# Find the word corresponding to an index using theget_itos()method. The result is a list where the index
# of the list corresponds to a word.
...
# Embedding layers
# An embedding layer is a crucial element in natural language processing (NLP) and neural networks designed for
# sequential data. It serves to convert categorical variables, like words or discrete indexes representing tokens,
# into continuous vectors. This transformation facilitates training and enables the network to learn meaningful
...
# Embeddings: Obtain the embedding for the first word with index 0 or 1. Don't forget that you have to convert
# the input into a tensor. The embeddings are initially initialized randomly, but as the model undergoes training,
# words with similar meanings gradually come to cluster closer together
...
# To establish the context, simply subtract 'j'. The size of the context is determined by the value ofCONTEXT_SIZE.
...
# Output the first element, which results in a tuple. The initial element represents the context,
# and the index indicates the following word.
...
#In this context, there are multiple words. Aggregate the embeddings of each of these words and then adjust the
# input size of the subsequent layer accordingly. Then, create the next layer.
embedding_dim=20
...
# Batch function
# Create a Batch function to interface with the data loader. Several adjustments are necessary to handle
# words that are part of a context in one batch and a predicted word in the following batch.
...
# To tackle this, the approach involves adjusting the final batch to conform to the specified batch size, ensuring it
# becomes a multiple of the predetermined size. When necessary, you'll employ padding techniques to achieve this
# harmonization. One approach you'll use is appending the beginning of the song to the end of the batch.
Padding=BATCH_SIZE-len(tokens)%BATCH_SIZE
tokens_pad=tokens+tokens[0:Padding]
Create theDataLoader.
...
# You have developed a PyTorch class for a multi-class neural network. The network's output is the probability of
# the next word within a given context. Therefore, the number of classes corresponds to the count of distinct words.
...
# While the model remains untrained, analyzing the output can provide us with a clearer understanding.
# In the output, the first dimension corresponds to the batch size, while the second dimension represents the
...
predicted_index
#tensor([77, 29, 60, 77, 17, 60, 17])

#Find the corresponding token.
[index_to_token[i.item()] for i in  predicted_index]
#['what', 'other', 'blind', 'what', 'lie', 'blind', 'lie']
...
# Training
# Training a language model involves a multi-step process that leverages training and testing data to optimize
# model performance. In the realm of Natural Language Processing (NLP), this process often employs various metrics
# to gauge a model's accuracy, such as perplexity or accuracy on unseen data. However, in the context of your current
# exploration, you will embark on a slightly different journey. Instead of relying solely on conventional NLP metrics,
# the focus shifts to manual inspection of the results.
...
# This code segment initializes an n-gram language model with a context size of 2
...
#The code provided below shows word embeddings from the created model, reduces their dimensionality to 2D using t-SNE,
# and then plots them as a scatter plot. Additionally, it annotates the first 20 points in the visualization
# with their corresponding words. This is used to visualize how similar words cluster together in a
# lower-dimensional space, revealing the structure of the word embeddings. Embeddings allow the model to
# represent words in a continuous vector space, capturing semantic relationships and similarities between words.
...
my_loss_list.append(my_loss)
#The code provided below shows word embeddings from the created model, reduces their dimensionality to 2d using t-SNE,
# and then plots them as a scatter plot. Additionally, it annotates the first 20 points in the visualization with
# their corresponding words. This is used to visualize how similar words cluster together in a lower-dimensional space,
# revealing the structure of the word embeddings. Embeddings allow the model to represent words in a continuous vector
# space, capturing semantic relationships and similarities between words.
...
my_loss_list.append(my_loss)
#The code provided below shows word embeddings from the created model, reduces their dimensionality to 2D using t-SNE,
# and then plots them as a scatter plot. Additionally, it annotates the first 20 points in the visualization with their
# corresponding words. This is used to visualize how similar words cluster together in a lower-dimensional space,
# revealing the structure of the word embeddings. Embeddings allow the model to represent words in a continuous
...
plt.legend(labels, loc='upper left', bbox_to_anchor=(1, 1))
plt.show()
#When considering the plotted loss for each model, a discernible trend emerges: an increase in context size correlates
# with a reduction in loss. While this specific approach lacks the inclusion of model validation or the utilization of
# conventional NLP evaluation metrics, the visual evidence substantiates its superior performance.
...
# It provides an indication of how well a model predicts a sample of data or the likelihood of an unseen event.
# Perplexity is commonly used in natural language processing tasks, such as machine translation, speech recognition,
