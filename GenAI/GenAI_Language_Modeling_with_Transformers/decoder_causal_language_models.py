...
# Text pipeline
#Dataset
#The code loads the IMDB dataset into training, and validation sets. It then creates an iterator for the training
# set and loops through the first 10 samples, printing each one. This process simulates how one might manually
# iterate over a dataset without using PyTorch's DataLoader for batch processing and data management.
...
next(data_itr)
# (1,
#  "If only to avoid making this type of film in the future. This film is interesting as an experiment but tells
#  no cogent story.<br /><br />One might feel virtuous for sitting thru it because it touches on so many
...
#  <br /><br />One might better spend one's time staring out a window at a tree growing.<br /><br />")
#
# define the device (CPU or GPU) for training. We'll check if a GPU is available and use it; otherwise, we'll use the CPU.
...
#test get_sample(block_size, text) first and get batch of texts:
BATCH_SIZE=1
...
#To test the get_sample function with a block size of 100, where the output includes both the source sequence and
# the target sequence, with the target sequence being the source sequence shifted by one character.
block_size=10
...
#The below code creates batches of source (`src_batch`) and target (`tgt_batch`) sequences from a dataset for
# training NLP models. It loops through the dataset to extract text samples, generates corresponding source and
# target sequences using the `get_sample` function, converts them into vocabulary indices, and then into PyTorch
# tensors. Each iteration appends these sequences to their respective batch lists and prints their details,
# including text, indices, and tensor shapes, for two samples per batch.
...
# The `collate_batch` function prepares batches of source and target sequences for training by
# processing each text sample in a given batch.
BLOCK_SIZE=30
...
#This process is useful for verifying that the data loader is functioning correctly and that t
# he sequences are being properly transformed.
...
print("target:",index_to_en(trt))
#move on to understanding the key components of the Transformer model.




#Masking
#In transformers, masking is crucial for ensuring certain positions are not attended to.
# The function generate_square_subsequent_mask produces an upper triangular matrix, which ensures
# that during decoding, a token can't attend to future tokens of target.
...
#check an example source tensor and its associated masks:
...
# Positional encoding
# The Transformer model doesn't have built-in knowledge of the order of tokens in the sequence.
# To give the model this information, positional encodings are added to the embeddings of the tokens.
# These encodings have a fixed pattern based on their position in the sequence.
...
## Token embedding
# Token embedding, also known as word embedding or word representation, is a way to convert words or tokens from a
# text corpus into numerical vectors in a continuous vector space. Each unique word or token in the corpus is
# assigned a fixed-length vector where the numerical values represent various linguistic properties of the word,
# such as its meaning, context, or relationships with other words.
...
# This class effectively encapsulates the necessary components to create a GPT-like model, allowing for
# training on language modeling tasks and text generation applications.
...
# After setting these hyperparameters, we create an instance of CustomGPTModel by passing in the embedding size,
# number of attention heads, number of layers, vocabulary size, and dropout probability.
# The model is then moved to the specified DEVICE, which could be a CPU or GPU, for training or inference.
...
# Prompting
#In order to get the model to generate text (next token), you will need to create an starting point,
# which we call prompt, for the model to append tokens to it and generate text. Verify that the prompt is
# neither None nor too long, then proceed to tokenize it, convert it into indices, and reshape as needed.
...
#see some differnt exmaples where the input is None or longer than block size:
...
#11 tokens per output, an additional batch dimension, along with corresponding logits values for each word in the vocabulary.
logits.shape
...
# In decoder models, we simply append the output to the input to generate the next response.
# We stop this process when we encounter the end-of-sequence tag <|endoftext|> or if the input becomes
# too large. We will implement it as a function later in this notebook.
...
#implement it as a function now
# Define special symbols and indices
UNK_IDX, PAD_IDX, EOS_IDX = 0, 1, 2
# Make sure the tokens are in order of their indices to properly insert them in vocab
special_symbols = ['<unk>', '<pad>', '<|endoftext|>' ]
...
#The key difference between the training and inference stages lies in the inputs to the decoder. During training,
# the decoder benefits from exposure to the ground truth--receiving the exact target sequence tokens incrementally
# through a technique known as "teacher forcing." This approach is in stark contrast to some other neural network
# architectures that rely on the network's previous predictions as inputs during training. Once training concludes,
# the datasets used resemble those employed in more conventional neural network models, providing a familiar
# foundation for comparison and evaluation.
...
#when calling model(src, src_mask, key_padding_mask), the forward method of the CustomGPTModel class generates
# logits for the target sequence, which can then be translated into actual tokens by taking the
# highest probability prediction at each step in the sequence.
...
# By following the aforementioned procedures, we can develop a function that is capable of making predictions
# and subsequently computing the corresponding loss on the validation data, we will use this fuction later on.
...
# use loss lists to keep track of our training and validation loss.
...
#can see that the result is not satisfactory, which is due to the fact that LLMs need to be trained on huge data
# for several epochs to be accurate.

#Loading GPT2 model from HuggingFace
#load the GPT2 model from HuggingFace to check how it performs at text generation:
